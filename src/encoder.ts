...
export class OSCEncoder {
  ...
  public encodeMessage(address: string, args?: OSCArgument[]): Buffer {
    if (!args || !args.length) {
  ...
  public encodeBundle(elements: OSCBundleElement[], timetag?: bigint): Buffer {
    return Buffer.concat([
  ...
  private encodeBundleElement(element: OSCBundleElement): Buffer[] {
  ...
  private writeStr(str: string): Buffer {
    return Buffer.from(str.concat('\0').padEnd(4 * Math.ceil((str.length + 1) / 4), '\0'));
  }

  private writeBlob(blob: Buffer): Buffer {
  ...
  private writeBigint(value: bigint): Buffer {
  ...
  private writeChar(value: string): Buffer {
  ...
  private writeUint(value: number): Buffer {
    const buf = Buffer.alloc(4);
    buf.writeUint32BE(value >>> 0);
    return buf;
  }

  private writeArg(arg: OSCArgument): Buffer | null {
    switch (arg.type) {
